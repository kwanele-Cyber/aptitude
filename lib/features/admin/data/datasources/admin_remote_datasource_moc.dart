...
class AdminRemoteDataSourceMock implements AdminRemoteDataSource {
  ...
  Future<List<PenaltyModel>> getPenalties({String? query}) async {
    await Future.delayed(const Duration(milliseconds: 400));
    var results = _mockPenalties;
    if (query != null && query.isNotEmpty) {
      final q = query.toLowerCase();
      results = results.where((p) => p.user.toLowerCase().contains(q) || p.reason.toLowerCase().contains(q)).toList();
    }
    return results;
  }
  ...
  Future<String> exportSystemData(String type) async {
    ...
    return 'https://example.com/exports/data_${DateTime.now().millisecondsSinceEpoch}.csv';
  }

  @override
  Future<SystemConfigModel> getConfig({String? query}) async {
    await Future.delayed(const Duration(milliseconds: 400));
    // Querying config is not usually needed, but kept for interface consistency
  ...
  @override
  Future<List<AuditLogEntryModel>> getAuditLogs({String? actor, String? action, int page = 1, int pageSize = 25}) async {
    await Future.delayed(const Duration(milliseconds: 400));
...
final _mockAuditLogs = [
  AuditLogEntryModel(id: 'l1', timestamp: DateTime.now(), actorId: 'a1', actorName: 'Admin_A', actorRole: 'Admin', action: 'Suspended user Kwanele Mhlongo (#2847)', detail: 'Reason: Violation of Rule 3', severity: 'critical', ip: '192.168.1.1', device: 'Chrome/Windows', changes: 'Status: Active → Suspended'),
  AuditLogEntryModel(id: 'l2', timestamp: DateTime.now(), actorId: 'a2', actorName: 'Admin_B', actorRole: 'Admin', action: 'Modified agreement #D-2026-0042 terms', detail: 'Change: Duration 8→12 weeks', severity: 'info', changes: 'Duration: 8 → 12 weeks'),
  AuditLogEntryModel(id: 'l3', timestamp: DateTime.now(), actorId: 'system', actorName: 'System', actorRole: 'System', action: 'User registered Thandi Nkosi (#2848)', detail: 'Email: thandi@example.com | Signup via: Google OAuth', severity: 'info'),
  AuditLogEntryModel(id: 'l4', timestamp: DateTime.now(), actorId: 'a1', actorName: 'Admin_A', actorRole: 'Admin', action: 'Login successful', detail: 'Admin_A authenticated successfully', severity: 'info', ip: '10.0.0.5', device: 'Chrome/Windows'),
  AuditLogEntryModel(id: 'l5', timestamp: DateTime.now(), actorId: 'a3', actorName: 'Admin_C', actorRole: 'Admin', action: 'Deleted review #1568 by Busi D', detail: 'Reason: Inappropriate content', severity: 'warning', changes: 'Review: visible → hidden'),
  AuditLogEntryModel(id: 'l6', timestamp: DateTime.now(), actorId: 'system', actorName: 'System', actorRole: 'System', action: 'Auto-flagged content in chat #2291', detail: 'Pattern: spam link detected', severity: 'warning'),
  AuditLogEntryModel(id: 'l7', timestamp: DateTime.now(), actorId: 'a2', actorName: 'Admin_B', actorRole: 'Admin', action: 'Applied penalty to Sipho Zulu', detail: 'Type: Warning | Strike: 1/3', severity: 'warning'),
  AuditLogEntryModel(id: 'l8', timestamp: DateTime.now(), actorId: 'a1', actorName: 'Admin_A', actorRole: 'Admin', action: 'Updated system config: match radius', detail: 'Changed from 30km to 50km', severity: 'info', changes: 'Match Radius: 30 → 50'),
  AuditLogEntryModel(id: 'l9', timestamp: DateTime.now(), actorId: 'a3', actorName: 'Admin_C', actorRole: 'Admin', action: 'Resolved dispute #DSP-2026-0089', detail: 'Decision: In favor of reporter', severity: 'info'),
  AuditLogEntryModel(id: 'l10', timestamp: DateTime.now(), actorId: 'system', actorName: 'System', actorRole: 'System', action: 'Daily backup completed', detail: 'Size: 2.4GB | Duration: 12m 34s', severity: 'info'),
];
